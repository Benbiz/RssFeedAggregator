...
public class FeedEntries {
	...
	public Response getAllEntries(@Context SecurityContext securityContext) {
		...
		for (Document docu : userfeedentries
				.find(new Document("userId", new ObjectId(securityContext.getUserPrincipal().getName())))) {
	...
	public Response getFeedEntries(@PathParam("id") String id, @Context SecurityContext securityContext) {
		...
		List<ObjectId> entries = new Vector<ObjectId>();
		Map<ObjectId, Boolean> read = new HashMap<>();
		for (Document docu : userfeedentries.find(new Document("feedId", feed.getObjectId("_id")).append("userId",
				new ObjectId(securityContext.getUserPrincipal().getName())))) {
			entries.add(docu.getObjectId("feedEntryId"));
			read.put(docu.getObjectId("feedEntryId"), docu.getBoolean("read"));
		}
	...
	public Response setRead(@FormParam("read") Boolean read, @PathParam("feedid") String feedid,
			@PathParam("entrieid") String entrieid, @Context SecurityContext securityContext) {
