...
public class Feeds {
    ...
    public Response addFeed(@FormParam("url") String url, @Context SecurityContext securityContext, @Context ServletContext context)  {
		...
		try
		{
            ...
            userfeed = new UserFeed(((PrincipalUser)securityContext.getUserPrincipal()).getUser(), feed);
            datastore.save(userfeed);
    		for (SyndEntry entry : syndfeed.getEntries())
    		{
    			feedentry = datastore.createQuery(FeedEntry.class)
    					.field("title").equal(entry.getTitle())
    					.field("link").equal(entry.getLink())
    					.field("feed").equal(datastore.getKey(feed))
    					.get();
                if (feedentry == null) {
    			...
    			userfeedentry = new UserFeedEntry(((PrincipalUser)securityContext.getUserPrincipal()).getUser(), feedentry);
    ...
    public Response getFeeds(@Context SecurityContext securityContext, @Context ServletContext context) {
		...
		Query<UserFeed> userfeeds = datastore.createQuery(UserFeed.class).field("user").equal(((PrincipalUser)securityContext.getUserPrincipal()).getUser());
	...
	@Path("{id}")
	@Produces(MediaType.APPLICATION_JSON)
    public Response getFeed(@PathParam("id") String id, @Context SecurityContext securityContext, @Context ServletContext context) {
		...
		if (feed == null)
			return Response.status(Response.Status.BAD_REQUEST).build();
		UserFeed userfeed = datastore.createQuery(UserFeed.class).field("user").equal(((PrincipalUser)securityContext.getUserPrincipal()).getUser())
				.field("feed").equal(feed).get();
	...
	@DELETE
	@Path("{id}")
    public Response deleteFeed(@PathParam("id") String id, @Context SecurityContext securityContext, @Context ServletContext context) {
		if (!ObjectId.isValid(id))
			return Response.status(Response.Status.BAD_REQUEST).build();
		...
		UserFeed userfeed = datastore.createQuery(UserFeed.class)
